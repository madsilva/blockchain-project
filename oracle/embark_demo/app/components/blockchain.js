...
import {Form, FormGroup, Input, HelpBlock, Button, FormText} 
from 'reactstrap';
...
class Blockchain extends React.Component {
  ...
  constructor(props) {
    super(props);
    var event = SimpleStorage.events.Request({}, '', function(error, result){console.log("event received");}).on('changed', (log) => {
  ...
  requestlistner = () => {
    SimpleStorage.events.Request('latest').then(function(result) {
      console.log("request received");
      this._addToLog("request received");
    });
  }
  ...
  async getValue(e) {
    ...
    //this._addToLog(JSON.stringify(obj))
    
    //SimpleStorage.methods.get().call().then(_value => this.setState({ valueGet: _value }));
    SimpleStorage.methods.request(12345,6789).call().then(result => console.log("event emitted"));
